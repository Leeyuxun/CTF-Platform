<?php 
        ...
        if((strpos($_REQUEST["flag23"],'/')!==false) or (strpos($_REQUEST["flag23"],'*')!==false) or (strpos($_REQUEST["flag23"],'$')!==false) or (strpos($_REQUEST["flag23"],'!')!==false) or (strpos($_REQUEST["flag23"],'^')!==false) or (strpos($_REQUEST["flag23"],'@')!==false) or (strpos($_REQUEST["flag23"],'#')!==false) or (strpos($_REQUEST["flag23"],'%')!==false) or (strpos($_REQUEST["flag23"],'(')!==false) or (strpos($_REQUEST["flag23"],'<')!==false) or (strpos($_REQUEST["flag23"],'>')!==false) or (strpos($_REQUEST["flag23"],'?')!==false) or (strpos($_REQUEST["flag23"],'\\')!==false) or (strpos($_REQUEST["flag23"],'+')!==false) or (strpos($_REQUEST["flag23"],'-')!==false) or (strpos($_REQUEST["flag23"],'=')!==false) or (strpos($_REQUEST["flag23"],'&')!==false) or (strpos($_REQUEST["flag23"],';')!==false) or (strpos($_REQUEST["flag23"],':')!==false) or (strpos($_REQUEST["flag23"],'"')!==false) or (strpos($_REQUEST["flag23"],'\'')!==false) or (strpos($_REQUEST["flag23"],'|')!==false) or (strpos($_REQUEST["flag23"],'[')!==false) or (strpos($_REQUEST["flag23"],']')!==false) or (strpos($_REQUEST["flag23"],'~')!==false) or (strpos($_REQUEST["flag23"],'`')!==false) or (strpos($_REQUEST["flag23"],'.')!==false))
		{
			?>
			<script type="text/javascript">
        ...
        if(error==0)
        {
            $qid23=htmlspecialchars(md5($_REQUEST["flag23"]));//获取html中的flag（通过post请求）
            ...
            $result=mysqli_query($con,"select * from achievement where exercise_num =23 and username='{$username}'");//查出对应用户名的信息
            if($result->num_rows)
            {
            ...
            else
            {
                ...
                $result=mysqli_query($con,"select flag from exercise where exercise_num =23");
                ...
                if($dbflag==$qid23)
                {
                    ...
                    $exercisetitle=mysqli_query($con,"select exercise_title from exercise where exercise_num=23");
                    ...
                    mysqli_query($con,"insert into achievement(exercise_num,exercise_title,username,score,atime) values(23,'$dbexercisetitle','$username',20,now())") or die("存入数据库失败2".mysqli_error($con)) ;
                    ?>
                    <script type="text/javascript">
